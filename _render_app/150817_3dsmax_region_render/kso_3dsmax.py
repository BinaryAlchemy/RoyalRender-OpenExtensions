...
def render_frame(FrStart,FrEnd,FrStep,arg):
    ...
    for frameNr in xrange(FrStart,FrEnd+1,FrStep):
        ...
        if (arg.regionEnabled):
            ...
            # Magoo: check the different region types
            if MaxPlus.RenderSettings.GetAreaType() == 1:
                renderCmdLine=renderCmdLine+" renderType: #selection "
            elif MaxPlus.RenderSettings.GetAreaType() == 2:
                renderCmdLine=renderCmdLine+" renderType: #region "
            elif MaxPlus.RenderSettings.GetAreaType() == 3:
                renderCmdLine=renderCmdLine+" renderType: #regionCrop "
            elif MaxPlus.RenderSettings.GetAreaType() == 4:
                renderCmdLine=renderCmdLine+" renderType: #blowUp "
            else:
...
# Magoo: This would disable any region settings, so we comment it out
